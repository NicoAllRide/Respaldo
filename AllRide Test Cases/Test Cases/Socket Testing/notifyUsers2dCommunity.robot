...
Start Departure Leg
    ...
    ...    data={"departureId":"${departureId}","communityId":"${idComunidad2}","startLat":-33.409703210605215,"startLon":-70.56737988971138,"customParamsAtStart":[],"preTripChecklist":[],"customParamsAtTheEnd":[],"routeId":"${scheduleId}","capacity":2,"busCode":"1111","driverCode":"159753","vehicleId":"666941a7b8d6ea30f9281110","shareToUsers":false,"customParams":[]}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
...
Validation QR With reservation
    ...
    ...    data={"communityId":"${idComunidad2}","validationString":"${qrCodeNoTickets}","timezone":"Chile/Continental","validationLat":-33.409703210605215, "validationLon":-70.56737988971138}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    Status Should Be    200
...
Link Validation to active Departure 1 With Custom
    ...
    ...    data={"plate":"MORISC","validations":["${validationIdNico}"]}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    Status Should Be    200
...
Validation QR With Tickets
    ...
    ...    mysesion
    ...    url=/api/v1/pb/provider/departures/validate
    ...    data={"communityId":"${idComunidad2}","validationString":"${qrCodeNico}","timezone":"Chile/Continental","validationLat":-33.409703210605215, "validationLon":-70.56737988971138}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    Status Should Be    200
