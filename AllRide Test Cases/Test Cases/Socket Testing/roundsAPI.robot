...
Three Full Laps In Segments (5s pacing)
    Create Session    gpssession    ${STAGE_URL}    verify=true

    ${headers}=    Create Dictionary
    ...    Authorization=${tokenDriver}
    ...    Content-Type=application/json

    ${plate}=    Set Variable    GPS DEVICE
    ${externalId}=    Set Variable    ext-987
    ...
    FOR    ${LAP}    IN RANGE    1    4
        Log To Console    ==== VUELTA ${LAP} : TRAMO A ====
        FOR    ${p}    IN    @{TRAMO_A}
            ${coords}=    Split String    ${p}    ,
            ${lat}=    Convert To Number    ${coords}[0]
            ${lon}=    Convert To Number    ${coords}[1]
            ${timestamp}=    Get Current Date    time_zone=UTC    result_format=%Y-%m-%dT%H:%M:%SZ
            ...
            ...    mileage=120000
            ...    accuracy=5
            ${payload['location']}=    Create Dictionary
            ...    latitude=${lat}
            ...    longitude=${lon}
            ${json_payload}=    Evaluate    json.dumps(${payload})    json
            ${response}=    POST On Session
            ...    gpssession
            ...    url=/api/v1/pb/external/gps
            ...    data=${json_payload}
            ...    headers=${headers}
            Log To Console    Enviado: ${json_payload}
            Log To Console    Respuesta: ${response.status_code}
            Sleep    5s
        END

        Log To Console    ==== VUELTA ${LAP} : TRAMO B ====
        FOR    ${p}    IN    @{TRAMO_B}
            ${coords}=    Split String    ${p}    ,
            ${lat}=    Convert To Number    ${coords}[0]
            ${lon}=    Convert To Number    ${coords}[1]
        ...
        Log To Console    ==== VUELTA ${LAP} : TRAMO C ====
        FOR    ${p}    IN    @{TRAMO_C}
            ${coords}=    Split String    ${p}    ,
            ${lat}=    Convert To Number    ${coords}[0]
            ${lon}=    Convert To Number    ${coords}[1]
        ...
        Log To Console    ==== VUELTA ${LAP} : TRAMO D ====
        FOR    ${p}    IN    @{TRAMO_D}
            ${coords}=    Split String    ${p}    ,
            ${lat}=    Convert To Number    ${coords}[0]
            ${lon}=    Convert To Number    ${coords}[1]
