...
Verify Open RDD in Community
    ...
    ...    ${responseJson}[custom][realTimeTransportSystem][buses][oDDServices][0][userRequests][freeRequests][enabled]
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
...
Get Active Departure Should be False
    # Define la URL del recurso que requiere autenticación (puedes ajustarla según tus necesidades)
    ${url}=    Set Variable
    ...    https://stage.allrideapp.com/api/v1/admin/pb/odd/${rddId}?community=653fd601f90509541a748683
    ...
    Should Be Equal As Numbers    ${response.status_code}    200
    Log    ${response.content}
    ${activeFalse}=    Set Variable    ${response.json()}[active]
    Should Be Equal As Strings    ${activeFalse}    False        active is not False, autoend departure not working