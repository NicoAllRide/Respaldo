...
Validate With QR(Kratos)
    ...
    ${response}=    POST On Session
    ...    mysesion
    ...    url=http://152.174.108.123:3000/api/v1/pb/provider/departures/validate
...
Action On Stop 2
    ...
    ${reserva_order_0}=    Evaluate    [r for r in ${reservations} if r["order"] == 1][0]
    ...
    ${reserva_order_1}=    Evaluate    [r for r in ${reservations} if r["order"] == 2][0]
    ...
    ${reserva_order_2}=    Evaluate    [r for r in ${reservations} if r["order"] == 3][0]
