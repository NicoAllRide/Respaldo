...
Action On Stop 1
    ...
    ${response}=    POST On Session
    ...    mysesion
    ...    url=/api/v2/pb/driver/oddepartures/action/654054d62d5b0119311ae116
    ...    data={}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    ...
    ${otras_reservas}=    Evaluate    [r for r in ${reservations} if r["order"] != 0]
    FOR    ${reserva}    IN    @{otras_reservas}
        Should Be Equal As Strings    ${reserva}[state]    pending
        ...    msg=❌ La reserva con order ${reserva}[order] debería estar en estado "pending" pero está en "${reserva}[state]"
    END
Action On Stop 2
    Set Log Level    TRACE
