...
Start Departure Leg
    ...
    ...    data={"communityId":"${idComunidad2}","startLat":-33.3908833,"startLon":-70.54620129999999,"customParamsAtStart":[],"preTripChecklist":[],"routeId":"${scheduleId}","capacity":3,"busCode":"1111","vehicleId":"${vehicleId2}","shareToUsers":false,"customParams":[],"pin":null}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    Status Should Be    200
...
Validate EXT
    ...
    # Configura las opciones de la solicitud (headers, auth)
    ${headers}=    Create Dictionary
    ...    Authorization=Bearer Nico86d174zd6e6f33b2cfu3a6m0c14cdf2ff32o3fbb56N7p10s4647bf62ad5ae2277ae6sb549e455b736n
    ...    Content-Type=application/json
    # Realiza la solicitud GET en la sesión por defecto
    ${response}=    POST On Session
    ...    mysesion
    ...    url=/ext/api/v1/departures/validate
    ...    data={"validationString":"be3655a0bd8479cfcdf46f8654f2d70341e2fe7428c9ac3a3933740575440ceef4c05aa02e4bef6b04324e25fe841ea1991391f3937b4234be7611bb8267bc0d666078059a5ece0ee6e95904","departureId":"68656a5bc282622e99dfc85c","validationLat":-38.75643,"validationLon":-73.66834}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
  # Capture key fields
    ...
    ...    msg=❌ Validation response is empty — expected validation data but received none.
    ${VALIDATION_ID}=    Set Variable    ${json}[_id]
    ${USER_ID}=          Set Variable    ${json}[userId]
    ${USER_NAME}=        Set Variable    ${json}[user][name]
    ${VALIDATED}=        Set Variable    ${json}[validated]
    ${REASON}=           Set Variable    ${json}[reason][0]

    Set Global Variable    ${VALIDATION_ID}
    Set Global Variable    ${USER_ID}

    # Check that the user was validated
    Should Be True    ${VALIDATED}
    ...    msg=❌ Expected the user to be validated, but got '${VALIDATED}'

    # Check that there is at least one reason and it matches "timestamp"
    Should Be Equal As Strings    ${REASON}    timestamp
    ...    msg=❌ Expected validation reason to be 'timestamp', but got '${REASON}'

    # Check user ID presence
    Should Not Be Empty    ${USER_ID}
    ...    msg=❌ userId is missing or empty

    # Check user name is not empty
    Should Not Be Empty    ${USER_NAME}
    ...    msg=❌ user.name is missing or empty

    # Validate coordinates
    Should Be Equal As Numbers    ${json}[latitude]    -38.75643
    ...    msg=❌ Expected latitude -38.75643 but got ${json}[latitude]

    Should Be Equal As Numbers    ${json}[longitude]    -73.66834
    ...    msg=❌ Expected longitude -73.66834 but got ${json}[longitude]
