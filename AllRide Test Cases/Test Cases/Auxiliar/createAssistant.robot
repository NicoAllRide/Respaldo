...
Create Auxiliar
    ...
    ...    mysesion
    ...    url=/api/v1/admin/pb/assistants?community=${idComunidad}
    ...    data={"name":"Auxiiliar RF2","email":"nicolas+auxiliarrf3@allrideapp.com","communities":["653fd601f90509541a748683"],"superCommunities":[],"ownerIds":[{"id":"653fd601f90509541a748683","role":"community"}],"password":"Lolowerty21"}
    ...    headers=${headers}
...
Resource Assignment(Driver and Vehicle)
    ...
    ...    data=[{"multipleDrivers":false,"driver":{"driverId":"${driverId}"},"drivers":[],"vehicle":{"assistantVehicleId":"${assistantVehicleId}","capacity":"${vehicleCapacity}"},"passengers":[],"departure":null}]
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    Should Be Equal As Numbers    ${code}    200
...
Start Departure PreLeg
    ...
    ...    data={"departureId":"${departureId}","communityId":"${idComunidad}","startLat":-33.3908833,"startLon":-70.54620129999999,"legType":"pre","customParamsAtStart":[],"preTripChecklist":[],"routeId":"${scheduleId}","capacity":3,"busCode":"1111","driverCode":"753","assistantVehicleId":"${assistantVehicleId}","shareToUsers":false,"customParams":[]}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    Status Should Be    200
...
Start Departure Leg
    ...
    ...    data={"departureId":"${departureId}","communityId":"${idComunidad}","startLat":-33.3908833,"startLon":-70.54620129999999,"customParamsAtStart":[],"preTripChecklist":[],"customParamsAtTheEnd":[],"routeId":"${scheduleId}","capacity":3,"busCode":"1111","driverCode":"753","assistantVehicleId":"${assistantVehicleId}","shareToUsers":false,"customParams":[]}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    Status Should Be    200
...
Validate With QR Assistant(Pending Assigned)
    #SE HACE CON DEPARTURE TOKEN#
    Create Session    mysesion    ${STAGE_URL}    verify=true
...
Link Validation to active Departure With QR
    ##
    #SE HACE CON ASSISTANt TOKEN#
    Create Session    mysesion    ${STAGE_URL}    verify=true
    ...
    ...    data={"validations":["${validationId}"],"vehicleQR":"${assistantVehicleId}"}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    Status Should Be    200
