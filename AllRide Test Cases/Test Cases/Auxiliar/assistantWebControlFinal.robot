...
Resource Assignment(Driver and Vehicle Without reservations assignment)
    ...
    Should Be Equal As Strings    ${routeId2}    66954794b24db9885e5aed7e        Departures created dont have the selected route, failing
...
Validate With QR Nico(Pending Assigned2)
    #SE HACE CON DEPARTURE TOKEN#
    Create Session    mysesion    ${STAGE_URL}    verify=true
...
Validate With QR Barbara(Pending Assigned2)
    #SE HACE CON DEPARTURE TOKEN#
    Create Session    mysesion    ${STAGE_URL}    verify=true
...
Validate With QR User 2 Should not be able, seat1 taken
    #SE HACE CON DEPARTURE TOKEN#
    Create Session    mysesion    ${STAGE_URL}    verify=true
    ...
    ...    mysesion
    ...    url=/api/v1/pb/provider/departures/validate
    ...    data={"validationString":"${qrCodeUser2}", "seatNumber": "1"}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    Status Should Be    403
    Log    ${response}
    Sleep    10s
Validate With QR User 2 Should not be able, seat2 taken
    #SE HACE CON DEPARTURE TOKEN#
    Create Session    mysesion    ${STAGE_URL}    verify=true
    ...
    ...    mysesion
    ...    url=/api/v1/pb/provider/departures/validate
    ...    data={"validationString":"${qrCodeUser2}", "seatNumber": "2"}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    Status Should Be    403
    Log    ${response}
    Sleep    10s
Validate With QR User 2 Should not be able,seat3 not existant
    #SE HACE CON DEPARTURE TOKEN#
    Create Session    mysesion    ${STAGE_URL}    verify=true
    ...
    ...    mysesion
    ...    url=/api/v1/pb/provider/departures/validate
    ...    data={"validationString":"${qrCodeUser2}", "seatNumber": "3"}
    ...    headers=${headers}
...
Validate With QR User 2 in departure2 (Should Pass)
    #SE HACE CON DEPARTURE TOKEN#
    Create Session    mysesion    ${STAGE_URL}    verify=true
    ...
    ${code}=    convert to string    ${response.status_code}
    Status Should Be    200
    Log    ${code}   ##Esto afirma que la validación no está completa, falta vincular
    ${validationIdUser2}=    Set Variable    ${response.json()}[_id]
    Set Global Variable    ${validationIdUser2}
    Sleep    10s
Validate With QR User 1 in departure2 (Should Fail, user dont have reservation or certification)
    #SE HACE CON DEPARTURE TOKEN#
    Create Session    mysesion    ${STAGE_URL}    verify=true
    ...
    ...    mysesion
    ...    url=/api/v1/pb/provider/departures/validate
    ...    data={"validationString":"${qrCodeUser1}"}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
...
Link Validation to active Departure 1 With Custom
    ...
    ...    data={"plate":"MORI2","validations":["${validationIdNico}", "${validationIdBarbara}"]}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    Status Should Be    200
    Log    ${code}
Link Validation to active Departure 2 With Custom
    #SE HACE CON ASSISTANt TOKEN -----------Duplicar validacion con el mismo usuasrio para luego vincular con patente de vehiculo
    Create Session    mysesion    ${STAGE_URL}    verify=true
    ...
    ${response}=    POST On Session
    ...    mysesion
    ...    url=/api/v1/pb/assistant/departures/link/${departureId_2}
    ...    data={"plate":"MORI3","validations":["${validationIdUser2}"]}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    Status Should Be    200
