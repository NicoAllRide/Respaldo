...
Login User Barbara(Azul -Gato Shouldbe able to see the route) 
...
Seat Reservation(User1-Nico Should be able to make reservation)
    Create Session    mysesion    ${STAGE_URL}    verify=true
    # Define la URL del recurso que requiere autenticación (puedes ajustarla según tus necesidades)
...
Seat Reservation(User2-Should not be able to make reservation EXPECTED ERROR)
    Create Session    mysesion    ${STAGE_URL}    verify=true
    # Define la URL del recurso que requiere autenticación (puedes ajustarla según tus necesidades)
...
Start Departure Leg
    ...
    ...    data={"departureId":"${departureId}","communityId":"${idComunidad2}","startLat":-33.3908833,"startLon":-70.54620129999999,"customParamsAtStart":[],"preTripChecklist":[],"customParamsAtTheEnd":[],"routeId":"${restrictionRoute}","capacity":46,"busCode":"171222","driverCode":"159159","vehicleId":"${vehicleId2}","shareToUsers":false,"customParams":[]}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    Status Should Be    200
...
Validate With QR(Nico) Should be able to validate
    ...
    Log    ${code}
    Sleep    10s
Validate With QR(Barbara) Should be Not able to validate
    Create Session    mysesion    ${STAGE_URL}    verify=true
