...
Login User (Nico Azul Perro, should not be able to see the route)
    Create Session    mysesion    ${STAGE_URL}    verify=true
    # Define la URL del recurso que requiere autenticación (puedes ajustarla según tus necesidades)
    # Configura las opciones de la solicitud (headers, auth)
...
Login User Barbara(Azul -Gato Should be able to see the route) 
...
Get Routes and Check "Restricciones RF" Should not be Present Nico
    # Define la URL del recurso que requiere autenticación (puedes ajustarla según tus necesidades)
    ...
    Should Not Contain    ${route_names}    Ruta Restricciones RF    Ruta Restricciones RF was found despite the user is not allowed

    Log    "Verification passed: 'Ruta Restricciones RF' not found in routes"


Get Routes and Check "Restricciones RF" Should be present
    # Define la URL del recurso que requiere autenticación (puedes ajustarla según tus necesidades)
    ${url}=    Set Variable    https://stage.allrideapp.com/api/v1/pb/user/routes/all/6654ae4eba54fe502d4e4187
    ...
    # Verificamos que "Restricciones RF" no esté en la lista de rutas
    Should Contain    ${route_names}    Ruta Restricciones RF    Ruta Restricciones RF was not found despite user dont have restriction
...
Get Routes and Check "Restricciones RF" Should be present User No match parameters
    ...
    # Verificamos que "Restricciones RF" no esté en la lista de rutas
    Should Contain    ${route_names}    Ruta Restricciones RF    Ruta Restricciones RF was not found despite user dont match any parameter
