...
Toggle Visibility Restriction only (Block to user)
...
Get Routes and Check "Restricciones RF" Not Present Nico
    ...
    Should Not Contain    ${route_names}    Ruta Restricciones RF    Ruta Restricciones RF was found despite the user restriction

    Log    "Verification passed: 'Ruta Restricciones RF' not found in routes"

Get Favorite and suggestedRoutes Nico "Restricciones RF" Not Present
    # Define la URL del recurso que requiere autenticación (puedes ajustarla según tus necesidades)
    ${url}=    Set Variable    https://stage.allrideapp.com/api/v1/pb/user/reservationsAndRoutes/6654ae4eba54fe502d4e4187
    ...
    ${favoriteRoutes}=    Set Variable    ${responseJson}[favoriteRoutes]

    # Extraemos los nombres de las rutas
    ${route_names}=    Evaluate    [route['name'] for route in ${favoriteRoutes}]    json
...
Get Routes and Check "Restricciones RF" Present User2
    ...
    # Verificamos que "Restricciones RF" no esté en la lista de rutas
    Should Contain    ${route_names}    Ruta Restricciones RF    Ruta Restricciones RF wasn't found, it should appear
