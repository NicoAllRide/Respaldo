...
Sync Departures Offline(Create departure, sync as active departure)
    ...
    ${offlineSync_synced}=    Set Variable        ${response.json()}[departures][0][offlineSync][internalId]
   ...
   # ${legs_customParamsAtEnd}=    Set Variable        ${response.json()}[departures][0][legs][0][customParamsAtEnd]
    ...
    ${customParamsAtStart}=    Set Variable        ${response.json()}[departures][0][customParamsAtStart]
    ${customParamsAtEnd}=    Set Variable        ${response.json()}[departures][0][customParamsAtEnd]
...
Sync Departures Offline(Create departure, sync as ended departure)
    Skip
    Create Session    mysesion    ${STAGE_URL}    verify=true
