...
Sync Departures Offline(Create departure, sync as active false departure)
    ...
    ${departureId2}=    Set Variable    ${response.json()}[departures][1][_id]
    ...
    ${departureToken2}=     Set Variable     ${response.json()}[departures][1][token]
    ...
    ${state_Active2}=    Set Variable        ${response.json()}[departures][1][active]
    Should Be Equal As Strings    ${state_Active2}    True
   ...
   # ${legs_customParamsAtEnd}=    Set Variable        ${response.json()}[departures][0][legs][0][customParamsAtEnd]
    ...
    Length Should Be    ${eventsLength}    4
...
Get Departure in admin, should be active false
    ...
    Log    ${response.content}
Get Departure in admin 2, should be active true
    # Define la URL del recurso que requiere autenticación (puedes ajustarla según tus necesidades)
    ${url}=    Set Variable
    ...    ${STAGE_URL}/api/v1/admin/pb/departures/${departureId2}?community=653fd601f90509541a748683
...
Stop Departure With Post Leg
    ...
    # Configura las opciones de la solicitud (headers, auth)
    ${headers}=    Create Dictionary
    ...    Authorization=Bearer ${departureToken2}
    ...    Content-Type=application/json
    # Realiza la solicitud GET en la sesión por defecto
    ${response}=    POST On Session
    ...    mysesion
