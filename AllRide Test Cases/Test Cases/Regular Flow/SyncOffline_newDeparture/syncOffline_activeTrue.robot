...
Sync Departures Offline(Create departure, sync as active departure)
   ...
   # ${legs_customParamsAtEnd}=    Set Variable        ${response.json()}[departures][0][legs][0][customParamsAtEnd]
    ...
    #--------------------Validations------------------------#
    ${validationLength}=    Set Variable    ${response.json()}[validations]
    ##Length Should Be    ${validationLength}    3  OMITIR POR MIENTRAS
    ${validation1_validated}=    Set Variable    ${response.json()}[validations][0][validated]
    Should Be Equal As Strings    ${validation1_validated}    True
    ${validation1_reason}=    Set Variable    ${response.json()}[validations][0][reason][0]
    Should Be Equal As Strings    ${validation1_reason}    manual_validation
    ${validation1_reason2}=    Set Variable    ${response.json()}[validations][0][reason][1]
    Should Be Equal As Strings    ${validation1_reason2}    offline_validation
    ${validation2_validated}=    Set Variable    ${response.json()}[validations][1][validated]
    Should Be Equal As Strings    ${validation2_validated}    True
    ${validation2_reason}=    Set Variable    ${response.json()}[validations][1][reason][0]
    Should Be Equal As Strings    ${validation1_reason2}    offline_validation
    ${validation3_validated}=    Set Variable    ${response.json()}[validations][2][validated]
    Should Be Equal As Strings    ${validation3_validated}    True
    ${validation3_reason}=    Set Variable    ${response.json()}[validations][2][reason][0]
    Should Be Equal As Strings    ${validation3_reason}    custom
    ${validation3_reason2}=   Set Variable    ${response.json()}[validations][2][reason][1]
    Should Be Equal As Strings    ${validation3_reason2}    offline_validation
    ${validation1_synced}=    Set Variable    ${response.json()}[validations][0][synced]
    Should Be Equal As Strings    ${validation1_synced}    True
    ${validation2_synced}=    Set Variable    ${response.json()}[validations][1][synced]
    Should Be Equal As Strings    ${validation2_synced}    True
    ${validation3_synced}=    Set Variable    ${response.json()}[validations][2][synced]
    Should Be Equal As Strings    ${validation3_synced}    True
    Log    ${code}
