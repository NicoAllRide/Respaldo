...
## Ver lista de productos disponibles desde la app https://stage.allrideapp.com/api/v2/products/catalog  {"promotions":false,"passes":true,"tickets":true}
## https://stage.allrideapp.com/api/v2/products/catalog/passes buscar por _id del producto recién creado
## Detalle https://stage.allrideapp.com/api/v2/products/68b5bd3de034867b455297d1
## Comprar producto 

Get Catalog (General)
    [Documentation]    Se verifica que se haya agregado correctamente el pase al usuario
    # Define the URL for authenticated resource
    ${url}=    Set Variable    ${STAGE_URL}/api/v2/products/catalog
...
Get Catalog (Passes)
    [Documentation]    Se verifica que se haya agregado correctamente el pase al usuario
    # Define the URL for authenticated resource
    ${url}=    Set Variable    ${STAGE_URL}/api/v2/products/catalog/passes
...
Get Pass Detail
    [Documentation]    Se verifica que se haya agregado correctamente el pase al usuario
    # Define the URL for authenticated resource
    ${url}=    Set Variable    ${STAGE_URL}/api/v2/products/${pass_id}
...
Get User QR(UserRobotFramework) 2
    ...
    Log    ${qrCodeNico2}
    Log    ${code}
Get User QR(UserRobotFramework) No passes
    Create Session    mysesion    ${STAGE_URL}    verify=true
    ...
    ${qrCodeNopasses}=    Set Variable    ${response.json()}[0][qrCode]
    Set Global Variable    ${qrCodeNopasses}
    Log    ${qrCodeNopasses}
    Log    ${code}
