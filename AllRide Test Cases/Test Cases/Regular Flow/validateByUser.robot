...
Validate by user in regular departure
    Create Session    mysesion    ${STAGE_URL}    verify=true
    ...
    ${response}=    POST On Session
    ...    mysesion
    ...    url=/api/v1/pb/user/departures/validate
    ...    data={"communityId":"${idComunidad2}","validationString":"67b5f76fb5c11f24f63fe29d"}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
    Status Should Be    200
    Log    ${code}
Validate by user in regular departure 1
    Create Session    mysesion    ${STAGE_URL}    verify=true
