...
Generate random UUID2 NO CARD EXISTING
    ${uuid_card_noExist}=    Evaluate    str(uuid.uuid4())
    Log    ${uuid_card_noExist}
    Set Global Variable    ${uuid_card_noExist}
Generate random UUID2 Pending Activation Card
    ${uuid_card_pendingActivation}=    Evaluate    str(uuid.uuid4())
    Log    ${uuid_card_pendingActivation}
    Set Global Variable    ${uuid_card_pendingActivation}
Generate random UUID2 Existing Card
