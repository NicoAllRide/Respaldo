...
Generate random UUID 2 (_id validations customValidations)
    ${uuid_custom2}=    Evaluate    str(uuid.uuid4())
    Log    ${uuid_custom2}
    Set Global Variable    ${uuid_custom2}
...
Sync validations offline
    ...
    ...    data={"validations":[{"assignedSeat":"3","communityId":"${idComunidad}","createdAt":"2024-06-28T15:48:27.139-04:00","departureId":"${departureId}","_id":"${uuid_qr}","isCustom":false,"isDNI":false,"isManual":false,"latitude":-34.394115,"loc":[-70.78126,-34.394115],"longitude":-70.78126,"qrCode":"${qrCodeNico}","reason":[],"remainingTickets":0,"routeId":"680bda0089a04026d7728f95","synced":false,"token":"","userId":"${idNico}","validated":true},{"assignedSeat":"3","communityId":"${idComunidad}","createdAt":"2024-06-28T15:48:27.139-04:00","departureId":"${departureId}","_id":"${uuid_qr}","isCustom":false,"isDNI":false,"isManual":false,"latitude":-34.394115,"loc":[-70.78126,-34.394115],"longitude":-70.78126,"qrCode":"${qrCodeNico}","reason":["offline_validation_limit"],"remainingTickets":0,"routeId":"680bda0089a04026d7728f95","synced":false,"token":"","userId":"${idNico}","validated":false}]}
    ...    headers=${headers}
    # Verifica el código de estado esperado (puedes ajustarlo según tus expectativas)
    ${code}=    convert to string    ${response.status_code}
...
Get Assigned Tickets After Validation(DNI)
    ...
    # Iteramos sobre los objetos de scheduledServicesA
    FOR    ${index}    IN RANGE    ${num_user_tickets}
        ${user}=    Set Variable    ${responseJson[${index}]}
...
# Ahora ya tienes el objeto electronicTicket y puedes trabajar directo
    ...
    Should Contain    ${paymentSettlement}    driverCode    No driverCode found
      Should Be Equal As Strings    ${driverCode}    753    driverCode should be 753 but it is ${driverCode}
